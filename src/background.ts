...
import { defaultColors, defaultParams } from "./types";

// Add default values to storage if they don't exist
(async function () {
  const { auditParameters, auditDisplay } = await chrome.storage.local.get(["auditParameters", "auditDisplay"]);

  if (!auditParameters || !auditDisplay) {
    await chrome.storage.local.set({
      auditParameters: defaultParams,
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "VALIDATE_OAUTH") {
    (async () => {
      try {
        ...
        await osu.API.createAsync(Number(auditOAuth.clientId), auditOAuth.clientSecret);
  ...
  if (request.action === "AUDIT") {
    (async () => {
      try {
        const { auditOAuth, auditParameters } = await chrome.storage.local.get(["auditOAuth", "auditParameters"]);
        ...
        const api = await osu.API.createAsync(Number(auditOAuth.clientId), auditOAuth.clientSecret);
        ...
        const averageGraveyardPlayCount =
          graveyard.length > 0 ? graveyardPlayCounts.reduce((sum, count) => sum + count, 0) / graveyard.length : 0;
...
chrome.webNavigation.onHistoryStateUpdated.addListener((details) => {
  chrome.tabs.sendMessage(details.tabId, { action: "REFRESH" });
});
